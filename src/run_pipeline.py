...
def _run_one_input(
    *,
    ...
    discarded_hard_path = os.path.join(out_dir, f"{pfx}discarded_hard.stage_final.jsonl")
    ...
    # NOTE: routing is driven by "min_votes_to_accept" (majority vote strength).
    ...
    if start_stage == "stage1":
        for r in normalized:
            ...
            stage1_okbad_by_uuid[uuid_key] = (ok1, n1 - ok1)
...
def main() -> None:
    ...
    for r in normalized:
        ...
        standardized = [standardize_choice_answer(a, choice_map=choice_map) for a in extracted]
        n1 = int(llm.stage_params("stage1_solve").n)
        vr = majority_vote(standardized[:n1])
        majority_answer = vr.majority
        majority_answer_json = {
            "majority": vr.majority,
            "majority_count": int(vr.majority_count),
            "counts": dict(vr.counts),
        }
    ...
    # Stage2/Stage3 can run even if Stage1 output files are not provided:
    # they use the original input rows for question/answer, and Stage1 status for routing.
    input_row_by_uuid: Dict[str, Dict[str, Any]] = {str(r.get("uuid")): r for r in normalized if r.get("uuid") is not None}
    ...
    for r in hard_rows:
        ...
        # Majority vote uses the same "boxed-first, then normalize" answers we judge with.
        s2_vote = majority_vote([str(a.get("normalized_answer") or "") for a in attempts][:n2])
        stage2_majority_answer = {
            "majority": s2_vote.majority,
            "majority_count": int(s2_vote.majority_count),
            "counts": dict(s2_vote.counts),
        ...
        append_jsonl_line(stage2_archive_path, entry)
        # Routing uses vote strength (consensus), not reference-accuracy ok/bad.
        next_stage = "stage3" if int(stage2_majority_answer.get("majority_count", 0)) < int(min_votes_to_accept) else "accepted"
        if next_stage == "accepted":
        ...
        stage2_done[uuid_key] = {"uuid": uuid, "ok": int(ok2_final), "bad": int(n2 - ok2_final), "next_stage": next_stage}
        if next_stage == "stage3":
            stage3_candidates.append(r)
    ...
    for r in stage3_candidates:
        ...
        # Majority vote uses the same "boxed-first, then normalize" answers we judge with.
        s3_vote = majority_vote([str(a.get("normalized_answer") or "") for a in attempts][:n3])
        stage3_majority_answer = {
            "majority": s3_vote.majority,
            "majority_count": int(s3_vote.majority_count),
            "counts": dict(s3_vote.counts),
        ...
        stage3_done[uuid_key] = {"uuid": uuid, "ok": int(ok3_final), "bad": int(n3 - ok3_final), "next_stage": next_stage}
